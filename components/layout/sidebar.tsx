...
export function Sidebar({ isAdmin }: { isAdmin: boolean }) {
  ...
  return (
    <aside className="group/sidebar hidden w-20 shrink-0 overflow-hidden border-r border-slate-900 bg-slate-950 text-white transition-[width] duration-300 ease-out lg:block hover:w-72 focus-within:w-72">
      <div className="flex h-24 items-center border-b border-white/10 px-4 lg:px-6">
        <div className="flex items-center gap-3">
          ...
          <div className="min-w-0 overflow-hidden opacity-0 transition-all duration-300 group-hover/sidebar:opacity-100 group-focus-within/sidebar:opacity-100 group-hover/sidebar:delay-75 group-focus-within/sidebar:delay-75">
        ...
        {items.map((item) => {
          ...
          return (
            <Link
              ...
              href={item.href}
              className={cn(
                "group flex h-12 items-center gap-3 rounded-xl px-3 text-sm font-semibold text-slate-300 transition hover:bg-white/10 hover:text-white lg:justify-center lg:px-0 lg:group-hover/sidebar:justify-start lg:group-hover/sidebar:px-3 lg:group-focus-within/sidebar:justify-start lg:group-focus-within/sidebar:px-3",
            ...
            >
              ...
              <span className="max-w-0 overflow-hidden whitespace-nowrap opacity-0 transition-all duration-300 group-hover/sidebar:max-w-[180px] group-hover/sidebar:opacity-100 group-focus-within/sidebar:max-w-[180px] group-focus-within/sidebar:opacity-100">
                {item.label}
              </span>
            </Link>
          );
