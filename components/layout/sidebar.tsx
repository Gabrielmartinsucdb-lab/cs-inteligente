...
export function Sidebar({ isAdmin }: { isAdmin: boolean }) {
  ...
  return (
    <aside className="group/sidebar hidden w-20 overflow-hidden border-r border-slate-900 bg-slate-950 text-white transition-[width] duration-300 ease-out lg:block lg:hover:w-72 lg:focus-within:w-72">
        ...
        {items.map((item) => {
          ...
          return (
            <Link
              ...
              href={item.href}
              className={cn(
                "group/item flex h-12 items-center gap-3 rounded-xl px-3 text-sm font-semibold text-slate-300 transition hover:bg-white/10 hover:text-white lg:justify-center lg:px-0 lg:group-hover/sidebar:justify-start lg:group-hover/sidebar:px-3",
            ...
            >
              <span className={cn(
                "flex h-8 w-8 items-center justify-center rounded-md bg-white/10 text-slate-200 transition group-hover/item:bg-white/15 group-hover/item:text-white",
                active && "bg-slate-950 text-white group-hover/item:bg-slate-950"
              )}>
